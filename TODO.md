
* Display small chords diagrams over each chord name.
    Note: Need to move frets one down on chord diagram (without changing position).
* Alternative exercise: Start each chord on a specific chord tone - Root, 3rd, 5th, 7th.
* Use Parcel free hosting to host the app.
* Add Strudel integration? 
    - Fix tempo
* Generate exercises based on songs (e.g., jazz standards, see songs/ folder).
* Port to React + TypeScript?
* Create licks by selecting less or more notes for each measure, with sustained or silent notes. 
