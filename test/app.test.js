...
function findClosestNote(previousNote, chordNotes, isAscending) {
  ...
  candidates.sort(
    (a, b) =>
      Math.abs(mockTonal.Note.midi(a) - prevMidi) -
      Math.abs(mockTonal.Note.midi(b) - prevMidi)
  );

  const candidateNote = candidates[0];
  const stepDiff = Math.abs(mockTonal.Note.midi(candidateNote) - prevMidi);
  if (stepDiff <= 2) {
    return candidateNote;
  }

  // Fallback: move to next note in the given direction
  const idx = chordNotes.indexOf(previousNote);
  let nextIdx = isAscending ? idx + 1 : idx - 1;
  if (nextIdx >= chordNotes.length) nextIdx = chordNotes.length - 1;
...
test('chord progression follows ascending/descending flow rules', () => {
  ...
  for (let measure = 0; measure < numMeasures; measure++) {
    ...
    const startNote =
      measure === 0
        ? chordNotes[Math.floor(chordNotes.length / 2)]
...
test('closest chord tone calculation works correctly', () => {
  ...
  const closestNote = findClosestNote(testNote, chordNotes, true);
...
describe('calculateMeasureWidth function', () => {
  ...
  beforeAll(() => {
    ...
    const functionMatch = flowJsContent.match(
      /function calculateMeasureWidth\([\s\S]*?\n}/
    );
    ...
    if (functionMatch) {
      ...
      throw new Error(
        'Could not extract calculateMeasureWidth function from flow.js'
      );
    }
  });
